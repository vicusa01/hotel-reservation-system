...
public class AdminMenu {
    ...
    private static void displayAllRooms() throws InterruptedException  {
    ...
    private static void PrintAllRooms(Collection<IRoom> rooms) throws InterruptedException {

        Thread singleRoomSortThread=new Thread(new Runnable() {
            @Override
            public void run() {
                rooms.stream().filter((x)->x.getRoomType() == RoomType.SINGLE)
                        .sorted((o1, o2)->o1.getRoomPrice().
                                compareTo(o2.getRoomPrice())).collect(Collectors.toList()).forEach(System.out::println);
            }
        },"vicusa");

        Thread doubleRoomSortThread=new Thread(new Runnable() {
            @Override
            public void run() {
                rooms.stream().filter((x)->x.getRoomType() == RoomType.DOUBLE)
                        .sorted((o1, o2)->o2.getRoomPrice().
                                compareTo(o1.getRoomPrice())).collect(Collectors.toList()).forEach(System.out::println);

            }
        },"vicusa2");

        singleRoomSortThread.start();
        singleRoomSortThread.join();

        doubleRoomSortThread.start();
        doubleRoomSortThread.join();
    }
